 ...
 */
class Digiboost_Theme_Includes {
	...
	public static function init() {
		...
		//theme path
		self::$template_directory = DIGIBOOST_THEME_PATH;

		self::$template_directory_uri = DIGIBOOST_THEME_URI;
		 ...
		 */
		{
			...
			if( defined('DIGIBOOST_ENABLE_WIDGET_ADDS' ) ) {
				//additional fields for widgets
				self::include_child_first( '/sub-includes/mods/mod-widget-adds.php' );
			}
			//for quick plugins editing
			if( ! empty ( DIGIBOOST_DEV_MODE ) ) {
				//no following constant. Temporary removing widget adds
					include_once( DIGIBOOST_THEME_PATH . '/dev/mwt-addons/mwt-addons.php' );
			}
	...
	public static function get_parent_path( $rel_path ) {
		return DIGIBOOST_THEME_PATH . self::get_rel_path( $rel_path );
	}
