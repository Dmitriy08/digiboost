...
if ( !function_exists('digiboost_action_in_widget_form') ) :
	function digiboost_action_in_widget_form( $t, $return, $instance ) {
		...
		<p class="widget_background_option">
			<label for="<?php echo esc_attr( $t->get_field_id( 'widget_background' ) ); ?>"><?php esc_html_e( 'Widget Background:', 'digiboost' ); ?></label>
			<select id="<?php echo esc_attr( $t->get_field_id( 'widget_background' ) ); ?>"
				...
				<option <?php selected( $instance['widget_background'], '' ); ?>
					value=""><?php esc_html_e( 'None', 'digiboost' ); ?></option>
				<option
					<?php selected( $instance['widget_background'], 'ls ms p-40' ); ?>value="ls ms p-40"><?php esc_html_e( 'Light Backgorund', 'digiboost' ); ?></option>
				<option
					<?php selected( $instance['widget_background'], 'ds ms p-40' ); ?>value="ds ms p-40"><?php esc_html_e( 'Muted Backgorund', 'digiboost' ); ?></option>
				<option <?php selected( $instance['widget_background'], 'ds p-40' ); ?>
					value="ds p-40"><?php esc_html_e( 'Dark Background', 'digiboost' ); ?></option>
                <option <?php selected( $instance['widget_background'], 'cs p-40' ); ?>
                        value="cs p-40"><?php esc_html_e( 'Accent color Background', 'digiboost' ); ?></option>
			</select>
		</p>
		<p class="widget_bootstrap_width">
			<label
				for="<?php echo esc_attr( $t->get_field_id( 'bootstrap_width' ) ); ?>"><?php esc_html_e( 'Widget Column Width:', 'digiboost' ); ?>
			</label>
			<select id="<?php echo esc_attr( $t->get_field_id( 'bootstrap_width' ) ); ?>"
			        name="<?php echo esc_attr( $t->get_field_name( 'bootstrap_width' ) ); ?>">
				<option <?php selected( $instance['bootstrap_width'], '' ); ?> value=""><?php esc_html_e( 'None', 'digiboost' ); ?></option>
				...
				<option <?php selected( $instance['bootstrap_width'], 'col-md-12' ); ?>value="col-md-12"><?php esc_html_e( 'Full Width', 'digiboost' ); ?></option>
			</select>
		</p>
	...
	} //digiboost_action_in_widget_form()
endif;

if( !function_exists('digiboost_filter_in_widget_form_update') ) :
	function digiboost_filter_in_widget_form_update( $instance, $new_instance, $old_instance ) {
		$instance['widget_background'] = $new_instance['widget_background'];
		$instance['bootstrap_width']   = $new_instance['bootstrap_width'];
	
		return $instance;
	} //digiboost_filter_in_widget_form_update()
endif;

if( !function_exists( 'digiboost_filter_dynamic_sidebar_params' ) ):
	function digiboost_filter_dynamic_sidebar_params( $params ) {
		...
		return $params;
	} //digiboost_filter_dynamic_sidebar_params()
endif;

//Add input fields(priority 5, 3 parameters)
add_action( 'in_widget_form', 'digiboost_action_in_widget_form', 5, 3 );
//Callback function for options update (priority 5, 3 parameters)
add_filter( 'widget_update_callback', 'digiboost_filter_in_widget_form_update', 5, 3 );
//add class names (default priority, one parameter)
add_filter( 'dynamic_sidebar_params', 'digiboost_filter_dynamic_sidebar_params', 1 );
