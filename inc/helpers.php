...
if ( !function_exists( 'digiboost_google_font_url' ) ) :
    function digiboost_google_font_url() {
...
endif; //digiboost_google_font_url()

if ( !function_exists( 'digiboost_add_font_styles_in_head' ) ) :
    function digiboost_add_font_styles_in_head() {
...
endif; //digiboost_add_font_styles_in_head()

if ( !function_exists( 'digiboost_custom' ) ) :
    function digiboost_custom() {
        if ( function_exists( 'fw_get_db_customizer_option' ) ) {
...
if ( !function_exists( 'digiboost_get_columns_classes' ) ) :
    /**
     * Define a sidebar position for manage main column CSS class, sidebar CSS class and visibility of sidebar.
     * return array
     */
    function digiboost_get_columns_classes( $full_width = false ) {
        ...
        if ( function_exists( 'fw_ext_sidebars_get_current_position' ) ) {
            ...
            if ( in_array( fw_ext_sidebars_get_current_position(), array( 'full' ) ) ) {
                ...
                if ( is_single() ) {
                    $column_classes['main_column_class'] = "col-12 col-xs-12 no-sidebar col-xl-8 offset-xl-2" . $main_column_class;
                    $column_classes['sidebar_class'] = false;
                    $column_classes['position'] = 'full';
                }
...
endif; //digiboost_get_columns_classes()

if ( !function_exists( 'digiboost_get_columns_classes_for_unyson_extended' ) ) :
    /**
     * Define a sidebar position for manage main column CSS class, sidebar CSS class and visibility of sidebar.
     * return array
     */
    function digiboost_get_columns_classes_for_unyson_extended( $full_width = false ) {
        
        // Sidebar Position
        
        // default
        $sidebar_position = apply_filters( 'digiboost_default_sidebar_position', 'right' );
        ...
        $empty = false;
        if ( apply_filters( 'digiboost_check_for_widgets', true ) ) {
        ...
        $s = apply_filters( 'digiboost_sidebar_width', 4 );             // sidebar width
...
endif; //digiboost_get_columns_classes_for_unyson_extended()
...
if ( !function_exists( 'digiboost_get_predefined_template_part' ) ) :
    /**
     * Return proper template part from options or default.
     * string $template_part_name
     */
    function digiboost_get_predefined_template_part( $template_part_name, $default_value = '1' ) {
...
endif; //digiboost_get_predefined_template_part()

//get ids of showing widgets
if ( !function_exists( 'digiboost_get_showing_widgets_ids' ) ) :
    /**
     * Return array of id's of all widgets that are showing.
     */
    
    function digiboost_get_showing_widgets_ids() {
...
endif; //digiboost_get_showing_widgets_ids

//returning first taxonomy of displayed archive or taxonomy
if ( !function_exists( 'digiboost_get_posts_single_taxonomy_name' ) ) :
    function digiboost_get_posts_single_taxonomy_name() {
...
endif; //digiboost_get_posts_single_taxonomy_name

//get all unique categories for all showing posts
if ( !function_exists( 'digiboost_get_post_categories' ) ) :
    function digiboost_get_post_categories( $taxonomy_name = 'category' ) {
...
endif; //digiboost_get_post_categories

//get all taxonomies slug for single post. Used inside loop
if ( !function_exists( 'digiboost_get_categories_slugs_for_single_post' ) ) :
    function digiboost_get_categories_slugs_for_single_post( $taxonomy_name = 'category' ) {
...
endif; //digiboost_get_categories_slugs_for_single_post
...
if ( !function_exists( 'digiboost_get_excerpt_for_page_with_unyson_builder' ) ) :
    function digiboost_get_excerpt_for_page_with_unyson_builder() {
...
endif; //digiboost_get_excerpt_for_page_with_unyson_builder

// check if is WooCommerce Page. Need for Customizer setting.
if ( !function_exists( 'digiboost_is_shop' ) ) :
    function digiboost_is_shop() {
        $res = false;
...
endif; //digiboost_is_shop

// check if is WooCommerce Products Page. Need for Customizer setting.
if ( !function_exists( 'digiboost_is_woocommerce_page' ) ) :
    function digiboost_is_woocommerce_page() {
        $res = false;
        if ( class_exists( 'WooCommerce' ) ) {
...
endif; //digiboost_is_woocommerce_page

//developer helper
if ( !function_exists( 'digiboost_var_export' ) ) :
    function digiboost_var_export( $var ) {
        echo '<pre>';
        var_export( $var );
        echo '</pre>';
    }
endif; //digiboost_var_export
...
if ( !function_exists( 'digiboost_shortcode_render' ) ) :
    /**
     * Render unyson shortcode if shortcode exists
     * @param string $shortcode_name
     * @param array $atts
     */
    function digiboost_shortcode_render( $shortcode_name, $atts ) {
        ...
        if ( defined( 'FW' ) && '' != $shortcode_name ) {
            ...
            if ( !empty( $shortcodes_extension ) ) {
                ...
                if ( !empty( $shortcode ) ) {
                    ...
                    echo fw()->extensions->get( 'shortcodes' )->get_shortcode( $shortcode_name )->render( $atts );
                }
            }
        }
    }
...
if ( !function_exists( 'digiboost_get_slide_speed' ) ):
    /**
     * Retrieve global sliding speed.
     */
    function digiboost_get_slide_speed( $default = 5000 ) {
...
if ( !function_exists( 'digiboost_slide_speed' ) ):
    ...
    function digiboost_slide_speed( $default = 5000 ) {
        ...
        echo esc_attr( digiboost_get_slide_speed( $default ) );
    }
endif;