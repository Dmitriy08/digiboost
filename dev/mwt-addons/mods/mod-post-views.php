...
if ( ! function_exists( 'mwt_show_post_views_count' ) ) :
	function mwt_show_post_views_count() {
		...
		if ( ! $number ) {
			$html = '<span class="item-views-count">0 '.esc_html__( 'Views', 'mwt' ) .'</span>';
		}

		if ( $number == 1 ) {
			$html = '<span class="item-views-count">1 '.esc_html__( 'View', 'mwt' ) .'</span>';
		}

		if ( $number > 1 ) {
			$html = '<span class="item-views-count">'.$number.' '. esc_html__( 'Views', 'mwt' ).'</span>';
		}
