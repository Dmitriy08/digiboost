...
if ( ! function_exists( 'mwt_post_likes_scripts' ) ) :
	...
	function mwt_post_likes_scripts() {
		...
		//theme
			...
			wp_enqueue_script( 'post-likes', DIGIBOOST_THEME_URI . '/dev/mwt-addons/mods/static/js/mod-post-likes.js', array( 'jquery' ), '1.0.0', true );
		}
...
if ( ! function_exists( 'mwt_post_like_button' ) ) :
	...
	function mwt_post_like_button( $postID ) {
		$output = '';
		if ( empty( $_COOKIE["$postID"] ) ) {
			$output = '<span data-id="' . $postID . '"><a href="" class="like_button like_active_button"><i class="fa fa-thumbs-up"></i></a></span>';
		} else {
