...
<div class="<?php echo esc_attr( $class ); ?>"  <?php echo wp_kses_post( $data_animation ); ?>>
	<?php
	if ( ! empty( $atts['column_padding_horizontal'] ) || ! empty( $atts['column_padding_vertical'] ) || ! empty( $atts['special_type_column'] ) || ! empty( $atts['background_color'] ) || ! empty( $atts['shadow'] )   ) : ?>
	<div class="<?php echo esc_attr( $atts['column_padding_horizontal'] . ' ' . $atts['column_padding_vertical']. ' ' . $atts['special_type_column'].' '.$atts['background_color'].' '.$atts['shadow'] ); ?>">
	<?php endif; //column_padding

		//showing column content
		echo do_shortcode( $content );

	if ( ! empty( $atts['column_padding_horizontal'] ) || ! empty( $atts['column_padding_vertical'] ) || ! empty( $atts['special_type_column'] ) || ! empty( $atts['background_color'] ) || ! empty( $atts['shadow'] ) ) : //closing optional column_padding div ?>
	</div>
	<?php endif; ?>
</div><!-- .col-* -->
